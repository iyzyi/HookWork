...
class CFrameProcessThreadDlg;		// 大坑点，CFrameNetworkDlg和CListNetworkData互相包含，必须提前声明


struct _ListProcessThreadRowData {
	CString			csIndex;
	CString			csFuncName;
	...
	_ListProcessThreadRowData* pNext = NULL;

	_ListProcessThreadRowData(DWORD dwIndex, CString csFuncName, DWORD dwErrorCode, CString csDetailInfo);
};
...
class CListProcessThreadData {
private:
	CFrameProcessThreadDlg* m_pMainDlg = NULL;
	CListCtrl* m_pListCtrl = NULL;

	_ListProcessThreadRowData* pFirst = NULL;
	_ListProcessThreadRowData* pLast = NULL;
public:
	_ListProcessThreadRowData** m_pRowDataIndexTable = NULL;

public:
	CListProcessThreadData(CFrameProcessThreadDlg* pMainDlg);
	~CListProcessThreadData();

	// 添加一行数据（不是向ListCtrl中添加，而是向我们自定义的CListData类中添加）如果没有过滤条件，则同时绘制此行到ListCtrl中
	void AddRow(DWORD dwIndex, CString csFuncName, DWORD dwErrorCode, CString csDetailInfo);

	// 在ListCtrl中绘制一行数据
	void DisplayRow(_ListProcessThreadRowData* pListProcessThreadRowData);
